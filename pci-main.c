...
static void pci_search_all_device(void);
        ...
        for (device_num = 0 ; device_num <= 31 ; device_num++) {            
            for (function_num = 0 ; function_num <= 7 ; function_num++) {
                ...
                if ((pci_read_config_reg16(&device.dev, 0x0)) == 0xFFFF) {
                    ...
                    /*
                      vendor_id = (WORD)tmp;
                      device_id = (WORD)(tmp >> 16);
                    */
                    
                    
                    /*
                      if((vendor = search_vendor_info(vendor_id, device_id))!=NULL){
                      _sys_printf(" pci%d: vendor:%x device:%x (%s)\n", i, vendor->vendor_id, vendor->device_id, vendor->name);
                      }else{
                ...
                /*
                  _sys_printf(" Bus:%d Dev:%d Fnc:%d MultiFunction:", bus_id, device_num, function_num);
                
                  if(check_multiple(bus_id, device_num, function_num)){
                  _sys_printf("Yes");
                  ...
                  print_device_type((BYTE)bus_id, (BYTE)device_num, (BYTE)function_num);
                
                  _sys_putchar('\n');
                
                  print_pci_resouce((BYTE)bus_id, (BYTE)device_num, (BYTE)function_num);
                
                  disable_pci_resource(bus_id, device_num, function_num);
...
void volatile pci_search_all_device(void)
{
